...
- Finished the SC12 proposal-store collapse: `company.teamProposals` is no longer a shared product model, and proposal memory now lives on CEO-board markdown task notes with only thin task metadata for approval/execution routing.
