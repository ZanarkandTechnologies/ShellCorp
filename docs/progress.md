...
- Rewrote `SC12` around a CEO-led team proposal workflow and then pivoted it to CLI-and-skill ownership: the new canonical path is `shellcorp team proposal` plus the CEO team CLI skill, User Tasks is now a thin founder review inbox over shared proposal state, and the CEO desk/menu open a dedicated CEO Workbench panel for proposal pipeline visibility.
