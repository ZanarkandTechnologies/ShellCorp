...
export function PlacementHandler() {
  ...
  const hasPendingTeamAssignment = Boolean(
    data && typeof data.pendingTeamId === "string" && data.pendingTeamId,
  );
  ...
  const handlePlacementSurfaceClick = useCallback(
    (event: ThreeEvent<MouseEvent>) => {
      if (!isCoordinatePlacement || pendingPosition || !hoverPositionRef.current) return;
      event.stopPropagation();
      setPendingPosition(hoverPositionRef.current);
    },
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Failed to place object. Please try again.";
  ...
  const confirmMsg: string =
    typeof prefab.placement.confirmMessage === "function"
            ...
            <Card>
              ...
              <CardContent>
                ...
                {prefab.placement.hint && (
                  <p className="text-xs text-yellow-600 mt-2">{prefab.placement.hint}</p>
                )}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                {data && typeof data.capacityError === "boolean" && data.capacityError ? (
                    ...
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div>
                        <p className="font-medium">
                          Cannot add desk to{" "}
                          <strong>
                            {typeof data.teamName === "string" ? data.teamName : "team"}
                          </strong>
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">
                          This team has reached the maximum capacity of{" "}
                          {typeof data.maxCapacity === "number" ? data.maxCapacity : "N/A"} desks.
                        </p>
                      </div>
                    </div>
                  </>
                  ...
                  <>
                    <p>
                      Add {prefab.displayName} to{" "}
                      <strong>
                        {data && typeof data.teamName === "string" ? data.teamName : "team"}
                      </strong>
                      ?
                    </p>
                    ...
                    {data && typeof data.remainingSlots === "number" && (
                      <p className="text-xs text-blue-600 mt-1">
                        {data.remainingSlots} slot{data.remainingSlots !== 1 ? "s" : ""} remaining
                ...
                {errorMessage && (
                  <div className="mt-3 flex items-start gap-2 rounded-md border border-destructive/20 bg-destructive/10 p-2">
